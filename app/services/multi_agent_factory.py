...
from app.services import get_prompt_service  # importar factory de prompt service
import logging
...
class MultiAgentFactory:
    ...
    def get_orchestrator(self, company_id: str) -> Optional[MultiAgentOrchestrator]:
        ...
        try:
            ...
            # -------------------------
            # Inyectar PromptService
            # -------------------------
            try:
                prompt_service = get_prompt_service()
                # set_prompt_service debe existir en MultiAgentOrchestrator
                if hasattr(orchestrator, "set_prompt_service"):
                    orchestrator.set_prompt_service(prompt_service)
                    logger.info(f"  → Prompt service injected for {company_id}")
                else:
                    logger.warning("MultiAgentOrchestrator has no method set_prompt_service(); skipping injection")
            except Exception as e:
                # No fallar el orquestador por problemas con prompts; loguear y continuar.
                logger.warning(f"Could not inject PromptService for {company_id}: {e}")
